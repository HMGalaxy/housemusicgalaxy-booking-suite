...
import { weddingForm, corporateForm, privateForm } from '../../shared/js/modules.js?v=10400';
...
function workflowStage(current='EVENT_FORM_PENDING'){
  const stages=[
    {label:'Consultation',states:['BOOKING_REQUEST_RECEIVED','EVENT_FORM_PENDING','EVENT_WORKBOOK_PENDING']},
    {label:'Quote & Contract',states:['QUOTE_PREPARATION','QUOTE_REVIEW','CONTRACT_PREPARATION','CONTRACT_DEPOSIT_PENDING']},
    {label:'Deposit',states:['DEPOSIT_REQUIRED','DEPOSIT_PENDING']},
    {label:'Event Planning',states:['PLANNING_FORM_PREPARATION','PLANNING_FORM_PENDING','PLANNING_REVIEW','PLANNING_PENDING']},
    {label:'Day of Event',states:['EVENT_READY','EVENT_COMPLETED']},
    {label:'Financials',states:['FINAL_PAYMENT_PENDING','PAID_COMPLETED']},
    {label:'Completed',states:['ARCHIVED']}
  ];
  const index=stages.findIndex(stage=>stage.states.includes(current));
  return {stages,index:index<0?0:index};
}
function workflow(e){const {stages,index}=workflowStage(e.event_data?.workflow?.currentState);return stages.map((stage,i)=>`<div class="progress-row ${i<index?'done':i===index?'current':''}"><i>${i<index?'✓':i+1}</i><span>${stage.label}</span></div>`).join('')}
