...
async function render() {
  ...
  const head = `<header class="topbar"><div><div class="eyebrow">Galaxy Cue Platform</div><h1>${view[0].toUpperCase() + view.slice(1)}</h1><p class="muted">Signed in as ${esc(user.email)}</p></div><span class="pill">v9.0.0</span></header>${data.errors.length ? '<div class="notice">Some platform data could not be read. Run the v7 platform migration and verify admin permissions.</div>' : ""}`;
